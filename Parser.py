...
class Parser:
    ...
    def parse(self, filePath,paths, columnNames,answerKeyPaths,updateBar):
        ...
        for myindex,path in enumerate(paths):
            ...
            quizName = self.parseColumn(df_head, 'Topic')[0]
            ...
            self.associateData(None,None,df,uName_cIndex,email_cIndex,date_cIndex,studentList,quizName,answerKeys)

            self.__oProducer.addIntoExecutionLog("Parsing of Poll file: " + path + " ended.")

        self.detectUncorStu(studentList)
        return studentList,self.__dataNotCorrelated,self.__stuNotCorrelated,answerKeys


    def associateData(self,student,uname,df,uName_cIndex,email_cIndex,date_cIndex,studentList,quizName,answerKeys):

        if uname != None:
            stuIndex = self.binarySearchStuList(student, studentList)
        ...
        self.__dataNotCorrelated[quizName] = pd.DataFrame(columns=df.columns.tolist())
            ...
            if uname!=None:
                ...
                for index, row in df_att.iterrows():
                    ...
                    if stuIndex != -1:
                        ...
                        stu = None
                        r = df_att.iloc[index, :]
                        self.__dataNotCorrelated[quizName].loc[index] = row.transpose()
        ...
        self.__dataNotCorrelated[quizName].drop(self.__dataNotCorrelated[quizName].columns[0], axis=1, inplace=True)
        self.__dataNotCorrelated[quizName] = self.__dataNotCorrelated[quizName].reset_index(drop=True)
        self.__dataNotCorrelated[quizName] = self.__dataNotCorrelated[quizName].reindex()
        self.detectUncorStu(studentList)
    ...
    def parseStudentList(self, filePath, name: str, surname: str, id: str):
        ...
        nameList = self.parseColumn(df, name)
        surnameList = self.parseColumn(df, surname)
        studentIdList = self.parseColumn(df, id)
